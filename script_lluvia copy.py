...
def get_pg_connection(retries=3, delay=5):
    """
...
def log_alert(descripcion):
    """
    Registra un error en la tabla alertas.control_alertas.
    """
    try:
        ...
        cur.close()
        conn.close()
        print("Alerta registrada en la base de datos (control_alertas).")
    except Exception as e:
        print(f"CRÍTICO: No se pudo registrar la alerta en la DB: {e}")
...
def cargar_por_fecha(fecha: date):
    ...
    records = fetch_rain_data(fecha, fecha)
...
def cargar_rango_fechas(fecha_inicio: date, fecha_fin: date):
    ...
    for dt in rrule(DAILY, dtstart=fecha_inicio, until=fecha_fin):
        dia = dt.date()
        print(f"  - Día {dia}")
        records = fetch_rain_data(dia, dia)
        insert_rain_records_into_db(records)
...
if __name__ == "__main__":
    ...
    try:
        if len(args) == 0:
            ...
            try:
                f_inicio = date.fromisoformat(args[0])
